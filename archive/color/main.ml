...
let parse () = ColorParser.main ColorLexer.token lexbuf



let color_to_formule () =
  let result = parse () in
  let (cf,f,assoc)=color result 4 in (**** ici la coloration 4 ***)
  
    print_string "p cnf ";
    print_int (Association.cardinal assoc);
    print_string " ";
    print_int (List.length cf);
    print_newline();
