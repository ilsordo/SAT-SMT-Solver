...
Afin d'observer l'existence d'une telle phase, nous avons produits des cnf à n=200 et l=3 fixés, pour k variant de 100 à 3000. L'algorithme utilisé est WL+DLCS. La courbe 1 indique les temps d'exécution obtenus. On peut observer que l'algorithme met le plus de temps à s'exécuter lorsque k est dans l'intervalle [700,1400]. En dehors de ces valeurs, les temps d'exécutions sont plus faibles. Courbe 2 figure le nombre de conflits suivant k. La courbe obtenue ressemble fortement à la précédente (les points absents correspondent à l'absence de conflits). Ceci confirme notre hypothèse, à savoir l'existence d'une phase de transition dans laquelle les cnf générées sont particulièrement difficiles à résoudre.
